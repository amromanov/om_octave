...
if exist('debug')&&debug==1
    ...
    switch linux
        case 1
            ...
            fprintf(fid,mosstring);
            fclose(fid);
            [~,~]=system('LD_LIBRARY_PATH= omc  +q +s simo.mos');
	          system(['cp ',modelname,'_init.xml ',modelname,'_init_original.xml']);

        case 0
            fid = fopen('simo.mos','w');
            fprintf(fid,mosstring);
            fclose(fid);
            system([win_om_path,'omc.exe +q +s simo.mos']);
            eval(['!copy ',modelname,'_init.xml ',modelname,'_init_original.xml']);
    end
  omcleanup(modelname);
end
